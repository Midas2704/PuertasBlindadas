...
async function sembrarM5() {
  await prisma.$transaction(async tx => {
    ...
    for (const nombre of ['DEMO M5 Servicios', 'DEMO M5 Materiales', 'DEMO M5 Gastos generales']) await tx.categoria_egreso_m5.upsert({ where: { nombre }, update: { activo: true }, create: { nombre } });
