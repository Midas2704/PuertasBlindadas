...
import { hashClave, futuro, politica, secreto, validarClave } from '../src/utilidades/seguridad';
...
export async function sembrarM4() {
 ...
 await prisma.$transaction(async tx=>{
  ...
  // Cuenta raíz de demostración: empleado activo, Gerencia y Administrador original.
  // El rol entrega los 74 CU, pero nunca omite validaciones de integridad o seguridad.
  const cargo=await tx.cargo.upsert({where:{nombre_cargo:'Demostración M4'},update:{estado_cargo:'activo'},create:{nombre_cargo:'Demostración M4',descripcion_cargo:'Cargo ficticio para la cuenta raíz local',estado_cargo:'activo'}});
  const vinculo=await tx.tipo_vinculo_laboral.upsert({where:{nombre_tipo_vinculo_laboral:'Demostración M4'},update:{estado_tipo_vinculo_laboral:'activo'},create:{nombre_tipo_vinculo_laboral:'Demostración M4',descripcion_tipo_vinculo_laboral:'Vínculo ficticio para la cuenta raíz local',estado_tipo_vinculo_laboral:'activo'}});
  await tx.empleado.upsert({where:{rut_empleado:acceso},update:{id_cargo:cargo.id_cargo,id_tipo_vinculo_laboral:vinculo.id_tipo_vinculo_laboral,nombres:'Midas',apellido_paterno:'Demostración',estado_laboral:'activo'},create:{rut_empleado:acceso,id_cargo:cargo.id_cargo,id_tipo_vinculo_laboral:vinculo.id_tipo_vinculo_laboral,nombres:'Midas',apellido_paterno:'Demostración',fecha_ingreso:new Date(),sueldo_base:0,estado_laboral:'activo'}});
  const cuenta=await tx.usuario.upsert({where:{acceso_m4:acceso},update:{empleado_m4:acceso,empleado_rut_empleado:acceso,usuario_username:'Midas',usuario_nombre_completo_primer_nombre_usuario:'Midas',usuario_rut_usuario:acceso,usuario_estado_cuenta:'activo',usuario_es_gerencia:true,gerencia:'Gerencia',usuario_es_administrador:true,administrador_original:true,configuracion_particular:false,perfil_id_perfil:perfiles.get('gerencia')!},create:{acceso_m4:acceso,empleado_m4:acceso,empleado_rut_empleado:acceso,usuario_username:'Midas',usuario_nombre_completo_primer_nombre_usuario:'Midas',usuario_rut_usuario:acceso,usuario_correo:'midas@puertasblindadas.local',usuario_estado_cuenta:'activo',usuario_es_gerencia:true,gerencia:'Gerencia',usuario_es_administrador:true,administrador_original:true,perfil_id_perfil:perfiles.get('gerencia')!,usuario_fecha_de_creacion:new Date()}});
  const vigente=await tx.usuario_contrasena.findFirst({where:{usuario_id_usuario:cuenta.usuario_id_usuario,activa:true,OR:[{vence:null},{vence:{gt:new Date()}}]},orderBy:{creada:'desc'}});
  if(!vigente) {
   claveInicialCreada=process.env.M4_CLAVE_INICIAL || secreto(); validarClave(claveInicialCreada);
   const hash=await hashClave(claveInicialCreada);
   await tx.usuario_contrasena.updateMany({where:{usuario_id_usuario:cuenta.usuario_id_usuario,activa:true},data:{activa:false,invalidada:new Date()}});
  ...
  await tx.estado_seguridad_usuario.upsert({where:{id_usuario:cuenta.usuario_id_usuario},update:{},create:{id_usuario:cuenta.usuario_id_usuario,responsable:'Seed inicial M4'}});
