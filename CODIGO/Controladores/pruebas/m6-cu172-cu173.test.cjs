...
test('M6 T4 CU172-CU173 integra Terreno por referencia sin adelantar CU futuros',async t=>{
 ...
 try{
  ...
  await t.test('permisos CU172 y CU173 son independientes y no existen CU174/CU208+',async()=>{
   assert.equal(codigosTodosLosCU.length,173);assert.equal(codigosTodosLosCU.includes('CU174'),false);assert.deepEqual(matrizPermisosPorCU.CU172,[]);assert.deepEqual(matrizPermisosPorCU.CU173,[]);
  ...
  await t.test('persistencia conserva referencia mínima y frontend separa acciones',()=>{
   ...
   assert.doesNotMatch(migracion,/DROP\s+(TABLE|COLUMN)|ALTER\s+TABLE\s+inventario/i);assert.match(correccion,/UNIQUE \(id_incidencia_retrabajo\)/);assert.doesNotMatch(correccion,/ON DELETE CASCADE/i);assert.match(vista,/puede172/);assert.match(vista,/puede173/);assert.match(app,/\['CU172','CU173'\]/);
  });
 }finally{
