...
test('sólo existen los controladores modulares autorizados y no hay llamadas entre módulos', () => {
  ...
  assert.deepEqual(readdirSync(carpeta).sort(), ['C_Finanzas.ts', 'M1Controller.ts', 'M2Controller.ts', 'M3Controller.ts', 'M4Controller.ts', 'M5Controller.ts', 'M6Controller.ts']);
  for (const nombre of ['M1Controller.ts', 'M2Controller.ts', 'M3Controller.ts', 'M4Controller.ts', 'M5Controller.ts', 'M6Controller.ts']) {
    const codigo = readFileSync(resolve(carpeta, nombre), 'utf8');
    assert.doesNotMatch(codigo, /from ['"].*(?:M[123456]Controller|C_Finanzas)['"]/);
    assert.doesNotMatch(codigo, /\$(?:queryRaw|executeRaw)|\.query\(|Router\(/);
  }
  const rutas = readFileSync(resolve('src/rutas/finanzas.ts'), 'utf8');
  assert.doesNotMatch(rutas, /prisma|M[123456]Controller/);
});
