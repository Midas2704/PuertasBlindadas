...
test('M4: sesiones, autorización, accesos y recuperación reales',async t=>{
 ...
 try{
  ...
  await t.test('login exitoso sin autorización previa y Administrador conserva todos los CU',async()=>{const r=await fachada.ejecutar('iniciarSesion',{cuerpo:{acceso:gerente.acceso_m4,clave},contexto:{}});token=r.token;assert.ok(token);actor=await modulo.autorizar('usuarios',{secretoSesion:token});assert.equal(actor.administrador,true);assert.equal(actor.permisos.length,74);assert.ok(actor.permisos.includes('CU01'));assert.ok(actor.permisos.includes('CU74'));});
  ...
  await t.test('múltiples sesiones permitidas temporalmente sin invalidar la primera',async()=>{const segunda=await ingresar(gerente);assert.ok(segunda.token);assert.equal((await modulo.autorizar('usuarios',{secretoSesion:token})).id,actor.id);assert.equal((await modulo.autorizar('usuarios',{secretoSesion:segunda.token})).id,actor.id);});
  ...
  await t.test('continuidad impide retirar el último administrador habilitado',async()=>{await assert.rejects(modulo.continuidad({usuario:{findMany:async()=>[]}}, {administrador_original:false,usuario_es_administrador:true,usuario_estado_cuenta:'activo',usuario_id_usuario:1n}),/Continuidad administrativa/);const original=await prisma.usuario.findFirstOrThrow({where:{administrador_original:true}});await assert.rejects(modulo.modificarUsuario('retirarAdministrador',actor,{id:original.usuario_id_usuario.toString(),confirmado:true}),/protegida|raíz Midas/);await assert.rejects(modulo.modificarUsuario('cambiarConfiguracion',actor,{id:original.usuario_id_usuario.toString(),configuracion:'gerencia',confirmado:true}),/raíz Midas/);});
