...
test('la matriz de perfiles conserva M1-M5 y deja M6 sin perfiles inventados',()=>{
  ...
  assert.deepEqual(codigosTodosLosCU,tramo(1,173));
  ...
  assert.deepEqual(Object.keys(matrizPermisosPorCU),tramo(1,173));
  for(const codigo of tramo(1,173)) assert.deepEqual(matrizPermisosPorCU[codigo],[...Object.keys(actores).filter(perfil=>actores[perfil].includes(codigo))]);
});
