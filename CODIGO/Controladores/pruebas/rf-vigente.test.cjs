...
test('la matriz de perfiles coincide con los actores de los 95 CU',()=>{
  const tramo=(desde,hasta)=>Array.from({length:hasta-desde+1},(_,i)=>`CU${String(desde+i).padStart(2,'0')}`);
  const actores={
    gerencia:tramo(1,95),
    secretaria:[...tramo(5,12),...tramo(14,22),...tramo(24,30),'CU32',...tramo(34,36),...tramo(39,40),...tramo(42,50),...tramo(55,56),...tramo(68,70),'CU76',...tramo(80,86),'CU88','CU89','CU90','CU91','CU92','CU93','CU95'],
    contador:[...tramo(5,11),...tramo(37,38),...tramo(42,58),...tramo(68,70),...tramo(80,88).filter(codigo=>!['CU89','CU90'].includes(codigo)),'CU93','CU95'],
  };
  ...
  assert.deepEqual(codigosTodosLosCU,tramo(1,95));
  ...
  assert.deepEqual(Object.keys(matrizPermisosPorCU),tramo(1,95));
  for(const codigo of tramo(1,95)) assert.deepEqual(matrizPermisosPorCU[codigo],[...Object.keys(actores).filter(perfil=>actores[perfil].includes(codigo))]);
});
